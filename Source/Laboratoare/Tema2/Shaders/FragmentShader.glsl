...
vec4 frontier()
{
    ...
    for(int i = -1; i <= 1; i++)
	{
		for(int j = -1; j <= 1; j++)
		{
            ...
            pixel_mat[i + 1][j + 1] =  0.21 * aux.r + 0.71 * aux.g + 0.07 * aux.b;
		}
	}
    ...
    for(int i = 0; i < 3; i++)
	{
		for(int j = 0; j < 3; j++)
		{
            gx_sum += pixel_mat[i][j] * Gx[i][j];
            gy_sum += pixel_mat[i][j] * Gy[i][j];
		}
	}
...
vec4 segmentation()
{
	...
	int offset = 256 / nrRegions;
	res.r = ((int(255.0f * color.r) / offset) * offset) / 255.0f;
	res.g = ((int(255.0f * color.g) / offset) * offset) / 255.0f;
	res.b = ((int(255.0f * color.b) / offset) * offset) / 255.0f;
