...
glm::vec3 Animation::InterpolateScaling(float animationTime, const std::string& boneName) {
	...
	if (isBlending) {
		if (animationTime >= blendingTime) {
			isBlending = false;
			prevAnim = nullptr;
		}
		else {
			float blendWeight = animationTime / blendingTime;
			return BlendScaling(blendWeight, boneName);
		}
	}
	animationTime = fmod(animationTime, duration);
	
	unsigned int keyIndex = UINT_MAX;
	if (animation.scalingKeys.size() == 0) {
		return glm::vec3(1);
	}
...
glm::quat Animation::InterpolateRotation(float animationTime, const std::string& boneName) {
	...
	if (isBlending) {
		if (animationTime >= blendingTime) {
			...
			float blendWeight = animationTime / blendingTime;
			return BlendRotation(blendWeight, boneName);
		}
	}
	animationTime = fmod(animationTime, duration);
	
	if (animation.rotationKeys.size() == 0) {
		return glm::quat();
	}
...
glm::vec3 Animation::InterpolatePosition(float animationTime, const std::string& boneName) {
	...
	if (isBlending) {
		if (animationTime >= blendingTime) {
			...
			float blendWeight = animationTime / blendingTime;
			return BlendPosition(blendWeight, boneName);
		}
	}
	animationTime = fmod(animationTime, duration);

	if (animation.positionKeys.size() == 0) {
		return glm::vec3(0);
	}
...
glm::vec3 Animation::BlendScaling(float blendWeight, const std::string& boneName) {
	glm::vec3 prev = prevAnim->InterpolateScaling(blendStartTime, boneName);
	AnimationNode& animation = boneAnimations[boneName];

	if (animation.scalingKeys.size() > 0) {
		glm::vec3 current = animation.scalingKeys[0].scale;
		return prev * (1.0f - blendWeight) + current * blendWeight;
	}
	else {
		return prev * (1.0f - blendWeight);
	}

	
}

glm::quat Animation::BlendRotation(float blendWeight, const std::string& boneName) {
	glm::quat prev = prevAnim->InterpolateRotation(blendStartTime, boneName);
	AnimationNode& animation = boneAnimations[boneName];

	glm::quat current = glm::quat();

	if (animation.rotationKeys.size() > 0) {
		current = animation.rotationKeys[0].rotation;
		return glm::slerp(prev, current, blendWeight);
	}
	else {
		return prev;
	}
...
glm::vec3 Animation::BlendPosition(float blendWeight, const std::string& boneName) {
	glm::vec3 prev = prevAnim->InterpolatePosition(blendStartTime, boneName);
	AnimationNode& animation = boneAnimations[boneName];
	glm::vec3 current = glm::vec3(0);

	if (animation.positionKeys.size() > 0) {
		glm::vec3 current = animation.positionKeys[0].position;
		return prev * (1.0f - blendWeight) + current * blendWeight;
	}
	else {
		return prev * (1.0f - blendWeight);
